...
class Computer
  ...
  def new_guess(gameboard)
    update_turn(gameboard)
    update_index(gameboard)
    update_guess(gameboard)
  ...
  def update_turn(gameboard)
    self.turn += 1 until gameboard.board[turn][:keys].include? 'empty'
  end

  def update_index(gameboard)
    self.index = 4 - gameboard.board[turn][:guess].count('empty')
  end

  def update_guess(gameboard)
    return if turn.zero? || !index.zero?

    previous_keys = gameboard.board[turn - 1][:keys]
